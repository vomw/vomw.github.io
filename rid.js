function ridjs(mode, cnt) {
  ...
  if (mode == 'banned') {
    el.src = `https://seep.eu.org/https://s.4cdn.org/image/error/${mode}/250/${mode}-${files[fid]}`;
    el.width = '250';
    el.setAttribute('style', 'float: right; padding-left: 10px; min-height: 150px;');
  }
  else {
    el.src = `https://seep.eu.org/https://s.4cdn.org/image/error/${mode}/${mode}-${files[fid]}`;
  }
